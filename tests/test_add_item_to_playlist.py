...
from tests.entity.test_data import LikeSong
...
class TestSearch(unittest.TestCase):
    ...
    def add_to_liked_list_teardown(self):
        self.home_page.click_save_to_library_button()

    def test_add_to_liked_list(self):
        self.home_page.click_search_button()
        self.home_page.insert_in_search_field(LikeSong.name.value + ' ' + LikeSong.artist.value)
        self.home_page.click_top_search_result()
        self.home_page.click_save_to_library_button()
        response = self.playlist_api.get_liked_songs()
        self.assertTrue(self.playlist_api.check_song_in_liked_playlist(response, LikeSong.name.value, LikeSong.artist.value))
        self.add_to_liked_list_teardown()
